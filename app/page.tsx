...
function BankModal({ film, onClose, onPaid, user, inline = false }: any) {
  ...
  const checkout = <>
    ...
    {isWalletTopup && <section className="wallet-topup-box" aria-label="Киноны данс цэнэглэх">
    ...
    {isWalletTopup && <button type="button" className="wallet-open-transfer" disabled={!orderReady} onClick={()=>setShowTransferDetails(true)}>
      <span><strong>Данс цэнэглэх</strong><small>{orderReady ? selectedTopup.toLocaleString()+"₮ шилжүүлэх мэдээлэл харах" : "Гүйлгээний утга үүсэж байна…"}</small></span>
      <span className="wallet-open-transfer-arrow" aria-hidden="true">›</span>
    </button>}
