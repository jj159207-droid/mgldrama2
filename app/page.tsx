      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",display:"flex",alignItems:"flex-end",zIndex:200}}>
        <div style={{background:C.card,borderRadius:"18px 18px 0 0",padding:"20px 20px 40px",width:"100%",border:`0.5px solid ${C.bd}`}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,marginBottom:16}}>
            <div style={{fontSize:26,fontWeight:800,color:C.gold}}>{film.price?.toLocaleString()}₮</div>
            <div style={{background:"#fff",borderRadius:8,padding:6}}>
              <QRCanvas text={`qpay:mgldrama:${film.id}:${film.price}:${selectedBank?.id}`} />
            </div>
            <div style={{fontSize:12,color:C.muted,textAlign:"center"}}>{selectedBank?.name}-ны QPay аппаар скан хийнэ үү</div>
          </div>
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",paddingBottom:30}}>
      ...
      <div style={{display:"flex",padding:"10px 14px",gap:8}}>
        <button onClick={()=>setTab("list")} style={{flex:1,padding:"10px",borderRadius:8,border:"none",background:tab==="list"?C.gold:C.card2,color:tab==="list"?"#000":C.muted,fontWeight:700,cursor:"pointer",fontSize:13}}>📋 Жагсаалт</button>
        <button onClick={()=>setTab("add")} style={{flex:1,padding:"10px",borderRadius:8,border:"none",background:tab==="add"?C.gold:C.card2,color:tab==="add"?"#000":C.muted,fontWeight:700,cursor:"pointer",fontSize:13}}>➕ Нэмэх</button>
      </div>
