...
export async function GET(req:NextRequest) {
  try {
    ...
    const [films,payments]=await Promise.all([
      db(`films?site_id=eq.${site}&select=id,title,badge&order=title.asc&limit=2000`),
      entitlementPayments(owner,site)
    ]);
    return json({films,access:accessFromPayments(payments),site});
...
export async function POST(req:NextRequest) {
  try {
    ...
    if(Object.keys(b).some(k=>!['user','plan','film_id','request_id'].includes(k))||typeof b.plan!=='string'||!(b.plan==='single'||b.plan==='3day'||Object.hasOwn(plans,b.plan)))
      throw new ApiError(400,'Багцаа сонгоно уу.');
    ...
    const [grant]=await db('rpc/kino_chat_grant_site','POST',{p_site:site,p_user:owner,p_plan:b.plan,p_film:film,p_request:chatRequestId(b.request_id)});
    await sendPushToUser(owner,site);
    return json({grant,site});
  } catch(error) {
    if(error instanceof ApiError&&error.code==='P0404')return json({message:'Энэ сайтын хэрэглэгч эсвэл кино олдсонгүй.'},404);
