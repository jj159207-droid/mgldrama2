...
import { db, fail, ApiError, canAdminSite, requestSite } from '@/lib/server';
import { chatId, chatSession } from '@/lib/chat';
export const runtime='nodejs';

export async function GET(req:NextRequest) {
  try {
    const site=requestSite(req),s=await chatSession(req),id=chatId(req.nextUrl.searchParams.get('id'));
    if(s.admin&&!canAdminSite(s,site))throw new ApiError(403,'Энэ сайтын админы эрх шаардлагатай.');
    const [msg]=await db(`support_messages?site_id=eq.${site}&id=eq.${id}${s.admin?'':`&user_id=eq.${s.userId}`}&select=id,has_image&limit=1`);
