...
const categories = [{key:'all',label:'Бүх кино'},{key:'gadaad',label:'Гадаад'},{key:'hyatad',label:'Хятад'},{key:'erotic',label:'Эротик'}];
...
export function ChatAdminActions({userId,phone,lastId,onChanged}: {userId:number;phone:string;lastId:number;onChanged:()=>void}) {
  const [panel,setPanel] = useState<'grant'|'clear'|null>(null), [busy,setBusy] = useState(false), [error,setError] = useState(''), [success,setSuccess] = useState('');
  ...
  return <div className="chat-admin-actions">
    <div className="chat-action-bar"><button type="button" onClick={()=>{setPanel('grant');setError('');setSuccess('');}} disabled={busy || !!pending}>＋ Эрх нээх</button>
      <button type="button" className="chat-delete" aria-label="Энэ чатыг устгах" disabled={!lastId || busy || !!pending} onClick={()=>{setThrough(lastId);setPanel('clear');setError('');setSuccess('');}}>Чат устгах</button></div>
    ...
    {panel==='grant' && <div className="chat-action-panel" aria-label="Хэрэглэгчийн эрх нээх">
      ...
      {pending ? <div className="chat-grant-confirm"><p><b>{pending.label}</b></p><p>Эрх одооноос эхэлнэ. Хэрэглэгчид чатанд мэдэгдэл очно.</p>
