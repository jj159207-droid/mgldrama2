...
function Copy-ClientSource([string]$TargetDir) {
    ...
    $excludedDirectories = @('.git', '.github', '.next', 'node_modules', 'installer', '.vercel', '.supabase', 'OUTPUT')
    $arguments = @($MasterRoot, $TargetDir, '/E', '/R:1', '/W:1', '/NFL', '/NDL', '/NJH', '/NJS', '/NP', '/XD') + $excludedDirectories + @('/XF', 'START-HERE.cmd', '.env', '.env.local', '*.log')
...
function Patch-ClientSource($Config, [string]$TargetDir) {
    ...
    $patched = [regex]::Replace($page, 'const DEFAULT_BANK_ACCOUNT = \{[\s\S]*?\};', $replacement, 1)
...
function Setup-GitHub($Config, [string]$TargetDir) {
    ...
    try {
        ...
        $null = Invoke-Capture -Command 'gh' -Arguments @('repo', 'create', $Config.RepoName, '--private', '--source', '.', '--remote', 'origin', '--push') -Label 'GitHub private repo'
...
function Ensure-SupabaseLogin {
    $probe = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'supabase@latest', 'projects', 'list', '-o', 'json') -Label 'Supabase auth probe' -AllowFail
    if ($probe.Code -ne 0) {
...
function Select-SupabaseOrganization {
    $response = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'supabase@latest', 'orgs', 'list', '-o', 'json') -Label 'Supabase organizations'
    ...
    if ($organizations.Count -eq 1) {
        return (Get-PropertyValue -Object $organizations[0] -Names @('id'))
    }

    Write-Host 'Supabase organization сонгоно:' -ForegroundColor Yellow
    for ($i = 0; $i -lt $organizations.Count; $i++) {
        $orgName = Get-PropertyValue -Object $organizations[$i] -Names @('name', 'slug', 'id')
        Write-Host "  $($i + 1). $orgName"
    }
    ...
    return (Get-PropertyValue -Object $organizations[$number - 1] -Names @('id'))
}
...
function Build-FreshMigrations($Config, [string]$TargetDir, [string]$WorkRoot) {
    ...
    $migrationSources = @(
        [pscustomobject]@{ Name = '20260916090001_core.sql'; Path = 'supabase\review-install.sql' },
        [pscustomobject]@{ Name = '20260916090002_https.sql'; Path = 'supabase\HTTPS-UPGRADE.sql' },
        [pscustomobject]@{ Name = '20260916090003_chat.sql'; Path = 'supabase\migrations\20260915183329_private_support_chat.sql' },
        [pscustomobject]@{ Name = '20260916090004_chat_actions.sql'; Path = 'supabase\migrations\20260915200520_chat_access_actions.sql' },
        [pscustomobject]@{ Name = '20260916090005_appearance.sql'; Path = 'supabase\migrations\20260916032117_site_appearance.sql' },
        [pscustomobject]@{ Name = '20260916090006_trailers.sql'; Path = 'supabase\trailers.sql' }
    )

    foreach ($item in $migrationSources) {
        Copy-MigrationFile -Source (Join-Path $TargetDir $item.Path) -Destination (Join-Path $migrationDir $item.Name)
    }
    ...
    Write-Utf8NoBom -Path (Join-Path $migrationDir '20260916090007_client_settings.sql') -Text "select * from public.kino_save_settings('$escapedSettings');`n"
...
function Wait-SupabaseProject([string]$ProjectRef) {
    ...
    for ($attempt = 0; $attempt -lt 72; $attempt++) {
        ...
        $response = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'supabase@latest', 'projects', 'list', '-o', 'json') -Label 'Supabase project status'
        $projects = @(ConvertFrom-LooseJson $response.Text)
        $project = $projects | Where-Object {
            (Get-PropertyValue -Object $_ -Names @('id', 'ref')) -eq $ProjectRef
        } | Select-Object -First 1

        if ($project) {
            $status = Get-PropertyValue -Object $project -Names @('status')
...
function Setup-Supabase($Config, [string]$TargetDir, [string]$DbPassword) {
    ...
    if (-not $organizationId) {
        throw 'Supabase organization ID олдсонгүй.'
    }

    $projectName = "$($Config.RepoName)-$(Get-Date -Format 'MMddHHmm')"
    $createResponse = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'supabase@latest', 'projects', 'create', $projectName, '--org-id', "$organizationId", '--db-password', $DbPassword, '--region', 'ap-southeast-1', '-o', 'json') -Label 'Supabase project create'
    $created = ConvertFrom-LooseJson $createResponse.Text
    $projectRef = Get-PropertyValue -Object $created -Names @('id', 'ref', 'project_ref')
    if (-not $projectRef) {
    ...
    try {
        ...
        Push-Location $workRoot
        try {
            $null = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'supabase@latest', 'link', '--project-ref', $projectRef, '--password', $DbPassword) -Label 'Supabase link'
            $null = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'supabase@latest', 'db', 'push', '--linked', '--password', $DbPassword, '--yes') -Label 'Supabase migrations'
        } finally {
            Pop-Location
        }
    } finally {
    ...
    $keysResponse = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'supabase@latest', 'projects', 'api-keys', '--project-ref', $projectRef, '-o', 'json') -Label 'Supabase API keys'
    ...
    $chosen = $keys | Where-Object {
        $name = Get-PropertyValue -Object $_ -Names @('name')
        $value = Get-PropertyValue -Object $_ -Names @('api_key', 'key', 'value')
        "$name" -match 'secret' -or "$value" -like 'sb_secret_*'
    } | Select-Object -First 1

    if (-not $chosen) {
        $chosen = $keys | Where-Object {
            (Get-PropertyValue -Object $_ -Names @('name')) -eq 'service_role'
        } | Select-Object -First 1
    }
    if (-not $chosen) {
    ...
    $keyValue = Get-PropertyValue -Object $chosen -Names @('api_key', 'key', 'value')
    $keyName = Get-PropertyValue -Object $chosen -Names @('name')
    if (-not $keyValue) {
        throw 'Supabase server key хоосон байна.'
    }

    $environmentName = 'SUPABASE_SERVICE_ROLE_KEY'
    if ("$keyName" -match 'secret' -or "$keyValue" -like 'sb_secret_*') {
...
function Ensure-VercelLogin {
    $probe = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'vercel@latest', 'whoami') -Label 'Vercel auth probe' -AllowFail
    if ($probe.Code -ne 0) {
...
function Get-VercelProductionUrl([string]$Text) {
    $aliases = [regex]::Matches($Text, '(?im)Aliased:\s*(https://[A-Za-z0-9.-]+\.vercel\.app)')
...
function Setup-Vercel($Config, [string]$TargetDir, $Supabase, [string]$AdminPassword, [string]$SmsSecret) {
    ...
    Push-Location $TargetDir
    try {
        $link = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'vercel@latest', 'link', '--yes', '--project', $Config.RepoName) -Label 'Vercel link' -AllowFail
        if ($link.Code -ne 0) {
        ...
        $gitConnect = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'vercel@latest', 'git', 'connect', '--yes') -Label 'Vercel Git connect' -AllowFail
        ...
        $firstDeploy = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'vercel@latest', '--prod', '--yes') -Label 'Vercel first production deploy'
        ...
        $finalDeploy = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'vercel@latest', '--prod', '--yes') -Label 'Vercel final production deploy'
        ...
        if ($Config.CustomDomain) {
            $domainResult = Invoke-Capture -Command 'npx' -Arguments @('--yes', 'vercel@latest', 'domains', 'add', $Config.CustomDomain, $Config.RepoName) -Label 'Vercel custom domain' -AllowFail
...
try {
    ...
    $siteUrl = Setup-Vercel @vercelArgs
    ...
    $macroPath = New-ClientMacroDroid @macroArgs
    ...
    Write-RecoveryFiles @recoveryArgs
    ...
    try {
        [System.Windows.Forms.MessageBox]::Show("Суулгалт амжилттай.`n$siteUrl`n`nOUTPUT → NEXT-STEPS.txt-ийг дагана уу.", 'TAZA SETUP') | Out-Null
    } catch {}
    ...
    try {
        Add-Type -AssemblyName System.Windows.Forms
        [System.Windows.Forms.MessageBox]::Show($_.Exception.Message, 'TAZA SETUP — Алдаа') | Out-Null
    } catch {}
    exit 1
}
