...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!smsText.includes("Khan Bank") || !smsText.includes("ORLOGO")) {
    ...
    await fetch(
      `${SUPABASE_URL}/rest/v1/pending_payments?ref_code=eq.${ref}`,
      {
        method: "PATCH",
        headers: {
          apikey: SUPABASE_KEY,
        ...
        body: JSON.stringify({ status: "confirmed", paid_amount: amount }),
      }
    );
